...
// VAPID — gere as chaves com: npx web-push generate-vapid-keys
// e coloque no .env como VAPID_PUBLIC_KEY e VAPID_PRIVATE_KEY
...
function getSubs(userId) {
  return readJSON(subFile(userId), []);
}

function saveSubs(userId, subs) {
  writeJSON(subFile(userId), subs);
}

// Envia push pra todos os dispositivos do usuário
export async function sendPush(userId, title, body) {
  ...
  // Remove subscriptions inválidas (dispositivo desinstalou o app etc)
  const valid = subs.filter((_, i) => results[i].status === 'fulfilled');
  if (valid.length !== subs.length) saveSubs(userId, valid);
}

// GET /push/vapid-key — frontend busca a chave pública
...
app.post('/push/subscribe', requireAuth, (req, res) => {
  ...
  if (!already) { subs.push(sub); saveSubs(req.user.id, subs); }
  res.json({ ok: true });
});
  ...
  for (const user of users) {
    try {
      ...
      if (msg) {
        ...
        await sendPush(user.id, 'Mind 🧠', msg);
      }
    } catch (e) {
